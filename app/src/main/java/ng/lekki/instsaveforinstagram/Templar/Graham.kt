...
import ng.lekki.instsaveforinstagram.R
import ng.lekki.instsaveforinstagram.Whiskey


class Graham : Service() {
    val clipBox = ArrayList<String>()


    companion object {
        val ACTION_PING = Graham::class.java.name + ".PING"
        val ACTION_PONG = Graham::class.java.name + ".PONG"
    ...
    fun publicAddressSystem() {
        ...
        val subHeading = getString(R.string.headings)
        val heading = getString(R.string.sub_headings)


        var builder: Notification.Builder? = null
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val ChannelId = "ng.lekki.instsaveforinstagram";
            val ChannelName = "InstDownload";
    ...
    override fun onBind(intent: Intent): IBinder? {
        ...
        override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            board.addPrimaryClipChangedListener {
                ...
                if (mainText.contains("instagram.com/p/")) {

                    if (!clipBox.contains(mainText)) {


                        val dialogIntent = Intent(this, Whiskey::class.java);
